...
安定状態になると、震度に応じたLEDが点灯し、過去の最大震度のLEDが点滅します(デフォルト: 10分でリセット)。

### EQIS-1

電源に接続すると、安定状態の確認が開始され `Stability checking...` という文字が表示されます。(英単語に自信がないので間違ってたら指摘ください…)  
OLED の寿命を考慮し、安定状態になると表示は消えます。  
揺れがあると OLED が点灯し、上部に震度が、右下に小さく計測震度が表示されます。  
過去の最大震度が更新されると、現在の震度の表示状況にかかわらず画面下部に `Max: 震度` が表示されます(デフォルト: 10分でリセット)。

まだまだ微妙な感じがしているので、アイデアをお待ちしております…。

### 表示設定

シリアルコマンド `DSPCFG` で、各ボードの表示に関する設定を変更できます。設定は EEPROM に保存され、再起動後も保持されます。

| 設定項目 | コマンド | 値の範囲 | デフォルト | 説明 |
| --- | --- | --- | --- | --- |
| 現在震度の表示閾値 | `DSPCFG C <値>` | 0-9 | 0 | この震度階級以上の場合に現在の震度を表示 |
| 最大震度の表示閾値 | `DSPCFG M <値>` | 0-9 | 0 | この震度階級以上の場合に最大震度を表示 |
| 最大震度のリセット時間 | `DSPCFG R <値>` | 1-1440(分) | 10 | 最大震度がリセットされるまでの時間 |

値と震度階級の対応: 0=震度0, 1=震度1, 2=震度2, 3=震度3, 4=震度4, 5=震度5弱, 6=震度5強, 7=震度6弱, 8=震度6強, 9=震度7

設定の照会は引数なしの `DSPCFG` で行えます。  
**この設定はディスプレイ表示にのみ影響し、NMEA 出力(`XSINT` 等)には影響しません。**
...
コマンド受信時に1回出力します。第1フィールドで設定カテゴリを識別します。  
`$XSCFG,カテゴリ,設定値...*チェックサム`

##### `DSP` 表示設定

`$XSCFG,DSP,現在震度の表示閾値,最大震度の表示閾値,最大震度のリセット時間(分)*チェックサム`
...
| `DSPCFG_INVALID` | DSPCFG コマンドのパラメータが不正 |
...
表示設定(`XSCFG`)を出力・変更します。詳しくは[表示設定](#表示設定)を参照してください。  
この機能に対応しているかは `DSPCFG` を送信し、応答の有無で判断できます。
